...
3. **Review Context**
   ...
   **Note:** When available, prefer MCP filesystem tools (`mcp__MCP_DOCKER__read_file`, `mcp__MCP_DOCKER__read_multiple_files`) for better performance and reliability when reading files.
...
4. **Identify Modified Files**
   ...
   **Note:** Use `mcp__MCP_DOCKER__directory_tree` or `mcp__MCP_DOCKER__list_directory` when available for efficient file discovery.
...
**Note:** When available, prefer MCP filesystem tools (`mcp__MCP_DOCKER__edit_file`, `mcp__MCP_DOCKER__write_file`) for file modifications to ensure better performance and reliability. Use `mcp__MCP_DOCKER__search_files` for efficient code pattern searching.
...
**Note:** When available, use `mcp__MCP_DOCKER__search_files` for more efficient pattern searching across the codebase.
