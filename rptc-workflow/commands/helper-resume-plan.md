...
### Resume from Handoff Checkpoint (NEW in v1.2.0)

```text
# TDD paused mid-execution due to context limits
# Handoff checkpoint was automatically generated
/rptc:helper-resume-plan "@user-authentication/"

# Output:
# - 🔄 Handoff checkpoint detected
# - Loads checkpoint state (paused after Step 3 of 7)
# - Configuration preserved (ultrathink mode, 85% coverage)
# - Next step content pre-loaded from handoff.md
# - Runs quick catch-up (only for codebase drift check)
# - Ready to continue with /rptc:tdd "@user-authentication/"
#
# Benefits:
# - Exact resume point (no guessing)
# - Context preserved (thinking mode, settings)
# - Reduced catch-up time (handoff already has context)
```

### Resume After Break (Standard Mode)

```text
# You worked on auth feature yesterday, continuing today
# No handoff checkpoint (completed steps manually)
/rptc:helper-resume-plan "@user-authentication/"

# Output:
# - 📋 No handoff checkpoint found
...
### Resume After Long Gap (Standard Mode)

```text
# Haven't touched plan in 2 weeks
/rptc:helper-resume-plan "@payment-processing/"
...
### Resume After Context Switch (Handoff Mode)

```text
# TDD paused for urgent bug, handoff generated
# Now returning to feature work
/rptc:helper-resume-plan "@analytics-dashboard/"

# Output:
# - 🔄 Handoff checkpoint detected
# - Paused at Step 6 of 7 (almost done!)
# - Handoff reason: "Context capacity management"
# - Configuration and context preserved
# - Runs quick catch-up
# - Shows final step: "Add export functionality"
# - Ready to finish quickly with exact checkpoint state
```
