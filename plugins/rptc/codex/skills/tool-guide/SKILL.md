...
Use `write_memory` to persist important discoveries for future sessions — things that would go in Codex memory or `AGENTS.md`:
...
Prefer over native web retrieval for web content retrieval:
...
Use `update_plan` when available for long sessions to track:
- Phases completed
- Files analyzed
