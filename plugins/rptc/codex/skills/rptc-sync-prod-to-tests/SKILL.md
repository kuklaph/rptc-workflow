...
**Examples:**
```bash
`rptc:rptc-sync-prod-to-tests`                    # Auto-detect everything
`rptc:rptc-sync-prod-to-tests` "src/auth/"        # Target specific subdirectory
`rptc:rptc-sync-prod-to-tests` --dry-run          # Analyze without changes
`rptc:rptc-sync-prod-to-tests` --max-iterations 5 # Limit iterations
```
...
**This skill REQUIRES delegation to specialist sub-agents via the spawn_agent tool.**
...
**All analysis and fixing work MUST be delegated to sub-agents using the spawn_agent tool.**
...
### Step 0f: Initialize update_plan
...
```json
{
  "plan": [
    {
      "step": "Sync tests for ${TARGET_DIR}",
      "status": "in_progress"
    },
    {
      "step": "Phase 1: Discover files",
      "status": "pending"
    },
    {
      "step": "Phase 2: SYNC LOOP",
      "status": "pending"
    },
    {
      "step": "Phase 3: Final test verification",
      "status": "pending"
    },
    {
      "step": "Phase 4: Generate report",
      "status": "pending"
    }
  ]
}
...
**Call update_plan:** Set "Phase 1: Discover files" as `in_progress`
...
### Step 1e: Update plan with Discovered Areas
...
```json
{
  "plan": [
    // ... existing todos ...
    {
      "step": "Analyze ${SUBDIR_1}",
      "status": "pending"
    },
    {
      "step": "Analyze ${SUBDIR_2}",
      "status": "pending"
    }
...
**Call update_plan:** Set "Phase 1: Discover files" as `completed`
...
**Call update_plan:** Set "Phase 2: SYNC LOOP" as `in_progress`
...
```json
{
  "plan": [
    ...
    {
      "step": "Iter 1: EXIT_CHECK",
      "status": "pending"
    },
    {
      "step": "Iter 1: PM_APPROVAL",
      "status": "pending"
    },
    {
      "step": "Iter 1: FIX",
...
**Call update_plan:** Set "Iter ${ITERATION}: EXPLORE" as `in_progress`

**⚠️ CRITICAL: MANDATORY SUB-AGENT DELEGATION**

**YOU MUST delegate analysis to `rptc:test-sync-agent` using the spawn_agent tool.**
...
Use the spawn_agent tool with agent_type: "rptc:test-sync-agent":
...
## SOPs to Reference
- RPTC plugin root/sop/test-sync-guide.md
...
- [ ] spawn_agent tool invoked with rptc:test-sync-agent for EACH subdirectory
...
**Call update_plan:** Set "Iter ${ITERATION}: EXPLORE" as `completed`

---

### Step 2.2: EXIT CHECK (Immediately After EXPLORE)

**Call update_plan:** Set "Iter ${ITERATION}: EXIT_CHECK" as `in_progress`
...
if [ "$ITERATION_GAPS_TOTAL" -eq 0 ]; then
  ...
  # Call update_plan to mark EXIT_CHECK completed, skip PM_APPROVAL/FIX/VERIFY
...
**Call update_plan:** Set "Iter ${ITERATION}: EXIT_CHECK" as `completed`

---

### Step 2.3: PM APPROVAL (Conditional - Production Changes Only)

**Call update_plan:** Set "Iter ${ITERATION}: PM_APPROVAL" as `in_progress`
...
**Capture PM decisions using request_user_input for EACH production bug:**

```json
{
  "questions": [{
    "id": "production_bug_decision",
    "header": "{issueId}",
    ...
    "options": [
      {"label": "Approve (Recommended)", "description": "Apply production fix (PM takes responsibility)"},
      {"label": "Reject", "description": "Keep production as-is, adapt test instead"},
      {"label": "Defer", "description": "Mark for manual review later"}
    ]
  }]
}
```
...
**Call update_plan:** Set "Iter ${ITERATION}: PM_APPROVAL" as `completed`

---

### Step 2.4: FIX (Fixer Agent Delegation)

**Call update_plan:** Set "Iter ${ITERATION}: FIX" as `in_progress`
...
**YOU MUST delegate fixes to `rptc:test-fixer-agent` using the spawn_agent tool.**
...
Use the spawn_agent tool with agent_type: "rptc:test-fixer-agent":
...
## SOPs to Reference
- RPTC plugin root/sop/testing-guide.md
...
- [ ] spawn_agent tool invoked with rptc:test-fixer-agent for each area with gaps
- [ ] FIX_RESULT parsed and metrics extracted
- [ ] Cumulative totals updated

**Call update_plan:** Set "Iter ${ITERATION}: FIX" as `completed`

---

### Step 2.5: VERIFY (Run Tests)

**Call update_plan:** Set "Iter ${ITERATION}: VERIFY" as `in_progress`
...
**Call update_plan:** Set "Iter ${ITERATION}: VERIFY" as `completed`
...
```json
{
  "plan": [
    ...
    {
      "step": "Iter ${ITERATION}: EXPLORE",
      "status": "pending"
    },
    {
      "step": "Iter ${ITERATION}: EXIT_CHECK",
      "status": "pending"
    },
    {
      "step": "Iter ${ITERATION}: PM_APPROVAL",
      "status": "pending"
    },
    {
      "step": "Iter ${ITERATION}: FIX",
      "status": "pending"
    },
    {
      "step": "Iter ${ITERATION}: VERIFY",
      "status": "pending"
    }
  ]
}
...
**Call update_plan:** Set "Phase 3: Final test verification" as `in_progress`
...
**Call update_plan:** Set "Phase 3: Final test verification" as `completed`

---

## Phase 4: Completion Report

**Call update_plan:** Set "Phase 4: Generate report" as `in_progress`
...
### Step 4c: Final update_plan Call
...
if [ $? -ne 0 ]; then
  ...
  # Use request_user_input to get user decision
fi
```
