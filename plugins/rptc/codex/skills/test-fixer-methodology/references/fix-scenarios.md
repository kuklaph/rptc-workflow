...
3. **Delegate new test generation** to TDD executor agent:
   ```markdown
   Request parent-session delegation to the RPTC TDD agent with `spawn_agent` when available:
...
**Fallback:** If Serena MCP is not available, use the available file-edit tool.
