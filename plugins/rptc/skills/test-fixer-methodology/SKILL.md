...
Test repair methodology for the RPTC test-fixer role. Orchestrates fix decisions and routes test generation through the provider-approved TDD executor path.
...
3. Use the provider's TDD executor path for new test generation: Claude may delegate to `rptc:tdd-agent`; Codex stays in the current session unless explicit delegation approval exists
...
4. Use the provider's TDD executor path for complete test generation: Claude may delegate to `rptc:tdd-agent`; Codex stays in the current session unless explicit delegation approval exists
...
**Your role**: Orchestrate — analyze sync report, gather context, route test writing through the provider's approved TDD executor path, integrate results, verify and retry.
...
- **Orchestrate, don't bypass TDD discipline** — Use the provider's approved TDD executor path for test writing
