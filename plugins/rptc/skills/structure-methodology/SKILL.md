...
| During RPTC feature workflow Phase 2 (Claude: `/rptc:feat`; Codex: `rptc-workflow` feature intent) | Design Guidance | Interface-first module design |
...
During RPTC feature workflow Phase 2 (Architecture), consider these questions:
...
When this skill is loaded by the architect-agent or code-review-agent during RPTC feature or verification workflows (Claude: `/rptc:feat` or `/rptc:verify`; Codex: matching `rptc-workflow` intents), it operates in **preventive mode**:
...
- If existing structure actively prevents clean implementation, suggest the user run the active provider's RPTC structure workflow as a separate step (Claude: `/rptc:structure`; Codex: `rptc-workflow` structure intent).

The goal is incremental improvement: each new feature is well-structured, and over time the codebase gets better through normal development. Wholesale restructuring is the job of the dedicated RPTC structure workflow, not inline planning.
