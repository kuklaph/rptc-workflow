...
That's it! The unified `/rptc:feat` workflow handles everything. In Claude, invoke it as a slash command. In Codex, invoke the equivalent `rptc-feat` skill through natural language.

Claude also supports `/rptc:feat-team` and `/rptc:fix-team` for persistent peer-agent workflows. Codex omits those commands because it uses parent-orchestrated `spawn_agent` delegation instead.
...
**After Discovery:** The workflow continues to Branch Strategy (current branch or new worktree), then Phase 2. In Claude, use `/rptc:feat-team` or `rptc:agent-teams` for peer/team flows. In Codex, use parent-orchestrated `spawn_agent` delegation where appropriate.
...
**What happens:**
1. Enters the provider planning mode
...
4. Consolidates findings and applies fixes via provider task tracker
...
**Purpose**: Configure RPTC in your project instruction file (`CLAUDE.md` or `AGENTS.md`).
...
**How it works:**
1. Detects current RPTC configuration state in project instruction file (`CLAUDE.md` or `AGENTS.md`)
2. Prompts for verification mode preference (automatic/all/minimal)
3. Inserts or updates RPTC configuration section at top of the project instruction file
...
### `/rptc:feat-team "description"` (Claude only)
...
### `/rptc:fix-team "bug description"` (Claude only)
...
RPTC uses the provider planning mode for planning:
- Plans stored in the active RPTC plan location for the current provider
...
Main context receives findings via provider task tracker and handles all fixes with user approval as needed.
...
3. Address findings via provider task tracker—main context handles all changes
...
- `claude/sop/todowrite-guide.md` - TodoWrite progress tracking patterns
