...
- Branch strategy options now show actual branch names: `Current branch [main]` and `New worktree [feature/xyz]` instead of generic labels
- Branch name generation moved before the AskUserQuestion so labels display real values
...
- Agent Teams support for all 8 agents: added SendMessage, TaskCreate, TaskUpdate, TaskList, TaskGet tools

### Fixed

- YAML frontmatter in code-review-agent, security-agent, and test-sync-agent (missing newline before `skills:` field)
